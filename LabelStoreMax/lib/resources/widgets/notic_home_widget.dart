...
class _NoticHomeWidgetState extends NyState<NoticHomeWidget> {
  ...
  _fetchCategories() async {
    if ((widget.wooSignalApp?.productCategoryCollections ?? []).isNotEmpty) {
      ...
      _categories = await (appWooSignal((api) => api.getProductCategories(parent: 0, perPage: 50, hideEmpty: true, include: productCategoryId)));
      ...
      _categories = await (appWooSignal((api) => api.getProductCategories(parent: 0, perPage: 50, hideEmpty: true)));
      _categories.sort((category1, category2) => category1.menuOrder!.compareTo(category2.menuOrder!));
    }
  }
  ...
  _showProduct(ws_product.Product product) => routeTo(ProductDetailPage.path, data: product);
}
