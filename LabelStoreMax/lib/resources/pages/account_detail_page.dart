...
class _AccountDetailPageState extends NyState<AccountDetailPage> with SingleTickerProviderStateMixin {
  ...
  _fetchWpUserData() async {
    ...
    try {
      wcCustomerInfoResponse = await WPJsonAPI.instance.api((request) => request.wcCustomerInfo());
  ...
  String getFullName() {
    return _wcCustomerInfoResponse?.data?.displayName ??
        [_wcCustomerInfoResponse?.data?.firstName, _wcCustomerInfoResponse?.data?.lastName].where((t) => (t != null || t != "")).toList().join(" ");
  }
