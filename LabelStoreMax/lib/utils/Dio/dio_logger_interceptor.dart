...
class DioLoggerInterceptor extends Interceptor {
  ...
  /// Print request data Options.product
  ...
  /// Constants
  final _kOkStatusEmoji = '✅';
  final _kErrorStatusEmoji = '🚫';
  ...
  void onError(DioError err, ErrorInterceptorHandler handler) {
    ...
    if (error) {
      if (err.type == DioErrorType.badResponse) {
        ...
        _printBoxed(
          header: 'DioError ║ Status: ${err.response?.statusCode} ${err.response?.statusMessage} ${_showEmoji(err.response?.statusCode)}',
          text: uri.toString(),
        );
  ...
  String _showEmoji(int? statusCode) {
    if (statusCode == null) {
      return _kErrorStatusEmoji;
    }
    if (statusCode >= 200 && statusCode <= 300) {
      return _kOkStatusEmoji;
    }
    return _kErrorStatusEmoji;
  }
    ...
    data.keys.toList().asMap().forEach((index, dynamic key) {
      ...
      if (value is Map) {
        ...
        if (compact && _canFlattenList(value)) {
          logPrint('║${_indent(_tabs)} $key: $value');
        } else {
          logPrint('║${_indent(_tabs)} $key: [');
